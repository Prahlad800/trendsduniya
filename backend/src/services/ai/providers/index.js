...
const chat = { build: compatible, read: r => ({text:r.choices?.[0]?.message?.content,usage:r.usage,truncated:r.choices?.[0]?.finish_reason==="length",blocked:!!r.choices?.[0]?.message?.refusal||["content_filter","error","tool_calls"].includes(r.choices?.[0]?.finish_reason)}) };
export const adapters = {
  ...
  gemini: {
    ...
    read: r => ({text:r.candidates?.[0]?.content?.parts?.filter(p=>!p.thought).map(p=>p.text||"").join(""),usage:r.usageMetadata,truncated:r.candidates?.[0]?.finishReason==="MAX_TOKENS",blocked:!!r.promptFeedback?.blockReason||!!r.candidates?.[0]?.finishReason&&!["STOP","MAX_TOKENS"].includes(r.candidates[0].finishReason)}),
  },
  anthropic: {
    ...
    read: r => ({text:r.content?.filter(p=>p.type==="text").map(p=>p.text).join(""),usage:r.usage,truncated:r.stop_reason==="max_tokens",blocked:["refusal","tool_use","pause_turn"].includes(r.stop_reason)}),
  },
};
