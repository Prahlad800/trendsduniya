...
export async function refreshTrends(country="IN",{force=false,providers=trendProviders}={}){
  ...
  if(!force&&snapshot.lastRefreshedAt&&now-snapshot.lastRefreshedAt<env.trendRefreshHours*3600000)return publicSnapshot(snapshot);
  ...
  const locked=await TrendingSnapshot.findOneAndUpdate({...key,$and:[{$or:[{lockUntil:{$exists:false}},{lockUntil:{$lte:now}}]},{$or:[{lastAttemptAt:{$exists:false}},{lastAttemptAt:{$lte:new Date(+now-60000)}}]}]},{$set:{lockToken,lockUntil:new Date(+now+60000),lastAttemptAt:now}},{returnDocument:"after"});
  ...
  try{
    ...
    const topics=rankTrends(items,now).map(t=>{const previous=snapshot.topics.find(p=>p.normalizedTitle===t.normalizedTitle);return {...t,...(previous?{_id:previous._id,firstSeenAt:previous.firstSeenAt}:{})};});
    const saved=await TrendingSnapshot.findOneAndUpdate({...key,lockToken},{$set:{topics,sourceHealth:health,topicsCollected:items.length,lastRefreshedAt:now,language:countries[country].locale}},{returnDocument:"after"});
