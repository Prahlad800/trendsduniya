...
import { publicConfig,saveConfig,testConfig,generateArticle } from "../services/ai/ai.service.js";
import { success } from "../utils/apiResponse.js";
import { audit } from "../services/audit.service.js";
export const getConfig=async(req,res)=>success(res,publicConfig(await AiConfig.findOne({singleton:"default"})));
...
export const putConfig=async(req,res)=>{const c=await saveConfig(req.body,req.admin);await audit(req,"AI_CONFIG_UPDATED",c,null,publicConfig(c));success(res,publicConfig(c),"Configuration saved");};
export const test=async(req,res)=>{
  try{const result=await testConfig(req.body);await audit(req,"AI_CONNECTION_TESTED",new AiConfig(),null,{provider:result.provider,model:result.model,success:true});success(res,result);}
